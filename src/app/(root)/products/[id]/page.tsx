...
import { getProductDetails } from '@/lib/actions/product';// adjust path
    ...
    <div>
      ...
      <div className="color-selector">
        ...
        <div className="colors">
          {data.availableColors.map((c) => (
            <a
              key={c.id}
              href={`/products/${id}?color=${c.slug}`}
              className={c.id === data.selectedColor?.id ? 'active' : ''}
      ...
      <div className="size-selector">
        ...
        <div className="sizes">
          {data.sizes.map((size) => (
            ...
            >
              {size.name}
              {size.inStock === 0 && ' (Out of Stock)'}
              {size.inStock > 0 && size.inStock <= size.lowStockThreshold && 
                ` (Only ${size.inStock} left)`}
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="images">
        {data.images.map((img) => (
          <img key={img.id} src={img.url} alt={data.product.name} />
        ))}
      </div>
